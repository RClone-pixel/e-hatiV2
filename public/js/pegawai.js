...
const PegawaiModule = {
    ...
    initPhotoLightbox: function () {
        ...
        document
            .getElementById("pegawaiLightboxClose")
        ...
        document.querySelectorAll(".pegawai-photo-thumb").forEach((img) => {
            img.addEventListener("click", () => {
                const src = img.src;
                const name =
                    img.getAttribute("data-name") || img.alt || "Foto Pegawai";
        ...
        document
            .querySelectorAll(".pegawai-photo-wrapper")
            .forEach((wrapper) => {
                const preview = wrapper.querySelector(
                    ".pegawai-photo-hover-preview",
                );
                if (!preview) return;
                ...
                wrapper.addEventListener("mouseenter", () => {
                    ...
                    const spaceBelow =
                        tableWrapperRect.bottom - wrapperRect.bottom;
    ...
    initTableSorting: function () {
        ...
        headers.forEach((header) => {
            ...
            header.addEventListener("click", () => {
                ...
                this.updateExportUrls(sortKey, isAsc ? "asc" : "desc");
            });
        });
    },

    updateExportUrls: function (sortKey, direction) {
        ...
        if (btnExcel) {
            const url = new URL(btnExcel.href);
            url.searchParams.set("sort", sortKey);
            url.searchParams.set("dir", direction);
            btnExcel.href = url.toString();
        }
    },
